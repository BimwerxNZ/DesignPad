...
namespace Calcpad.OpenXml
{
    public class OpenXmlWriter
    {
        ...
        public string Convert(string html, string path)
        {
            ...
            // Validate the written document (optional)
        ...
        private static void CopyRunProperties(OpenXmlElement source, OpenXmlElement dest)
        {
            ...
            destProps.Color ??= sourceProps.Color?.Clone() as DocumentFormat.OpenXml.Wordprocessing.Color;
        ...
        private OpenXmlElement Node2Element(HtmlNode domNode, MainDocumentPart mainPart)
        {
            ...
            switch (nodeName)
            {
                ...
                case "svg":
                    ...
                    //{
                    //    // ---------------------------------
                    //    // 1) Figure out the local path (src)
                    //    // ---------------------------------
                    //    var src = domNode.GetAttributeValue("src", string.Empty);

                    //    // If you have a base URL or relative path logic, handle it here.
                    //    // Possibly combine with _url if needed:
                    //    // src = Path.Combine(_url, src); // if that makes sense in your scenario

                    //    //// ---------------------------------
                    //    //// 2) Actually get the file's real dimensions
                    //    //// ---------------------------------
                    //    //double fileWidthPx = 0;
                    //    //double fileHeightPx = 0;
                    //    //GetActualImageSizeFromFile(src, out fileWidthPx, out fileHeightPx);

                    //    //// If we still have 0, fallback to some default
                    //    //if (fileWidthPx <= 0 || fileHeightPx <= 0)
                    //    //{
                    //    //    fileWidthPx = 600;
                    //    //    fileHeightPx = 400;
                    //    //}
                    ...
                    //    //// ---------------------------------
                    //    //// 3) A4 + margin + 90% usage
                    //    //// ---------------------------------
                    //    //const double A4WidthInches = 8.27;
                    //    //const double marginInches = 1.0;
                    //    //const double usageRatio = 0.90;
                    //    //double usableWidthInches = (A4WidthInches - 2 * marginInches) * usageRatio;

                    //    //// 1 inch = 914,400 EMUs
                    //    //const double emuPerInch = 914400;
                    //    //double maxWidthEmu = usableWidthInches * emuPerInch;

                    //    //// ---------------------------------
                    //    //// 4) Convert from pixel to EMUs
                    //    ////    (1 pixel = 9,525 EMUs for Word)
                    //    //// ---------------------------------
                    //    //const double emuPerPixel = 9525;
                    //    //double widthEmu = fileWidthPx * emuPerPixel;
                    //    //double heightEmu = fileHeightPx * emuPerPixel;
                    ...
                    //    //{
                    //    //    double scaleFactor = maxWidthEmu / widthEmu;
                    //    //    widthEmu = maxWidthEmu;
                    //    //    heightEmu = heightEmu * scaleFactor; // preserve aspect ratio
                    //    //}

                    //    //// ---------------------------------
                    //    //// 6) Clamp to int (OpenXML limit)
                    //    //// ---------------------------------
                    //    //int finalWidthEmu = (int)Math.Min(widthEmu, int.MaxValue);
                    //    //int finalHeightEmu = (int)Math.Min(heightEmu, int.MaxValue);

                    //    //finalWidthEmu = (int)(widthEmu * 0.000125);
                    //    //finalHeightEmu = (int)(heightEmu * 0.000125);

                    //    // ---------------------------------
                    //    // 7) Alt text fallback (or read from HTML)
                    //    // ---------------------------------
                    //    var alt = domNode.GetAttributeValue("alt", "Image");

                    //    // ---------------------------------
                    //    // 8) Call your code that adds the image
                    //    // ---------------------------------
                    //    return ImageWriter.AddImage(
                    //        src,
                    //        _url,
                    //        alt,
                    //        600,
                    ...
                    catch (Exception e)
                    {
                        return new Paragraph(new Run(
                            new Text("Error adding image: " + e.Message)
                        ));
                    }
        ...
        private static void GetActualImageSizeFromFile(
    string imagePath,
    out double widthPx,
    out double heightPx)
        {
            ...
            if (!File.Exists(imagePath))
                return; // failsafe if the file does not exist
            ...
            {
                // If reading fails for any reason, just leave them as 0
            }
        }
        ...
        private static OpenXmlElement AddHr(HtmlNode domNode)
        {
            ...
            if (name == "p" || name[0] == 'h' && name.Length == 2)
            {
                ...
                {
                    RunProperties = new(new DocumentFormat.OpenXml.Wordprocessing.Color() { Val = hrColor })
                };
            }
            Run r = new(new Text("."))
            {
        ...
        private static DocumentFormat.OpenXml.Wordprocessing.Color MakeColor(string color) =>
            new()
            {
                Val = color,
            };
